...
- **Type:** `string`
- **Default:** `'Read the documentation at {url} and help me understand it.'`

Default prompt sent to every "Open in…" provider that doesn't define its own
[`providers.<id>.prompt`](#providers). Placeholders:

- `{url}` — substituted with the absolute URL of the current page.

The resolved prompt (with `{url}` replaced) is what gets URL-encoded into
provider URL templates and copied to the clipboard for `clipboard-open`
providers.

```js
starlightLlmActions({
  prompt: 'Read this Acme docs page and answer in plain English: {url}',
})
```
...
- `{prompt}` — the resolved prompt (with `{url}` substituted).
