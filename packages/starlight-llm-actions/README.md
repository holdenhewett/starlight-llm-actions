...
- Content Collections beyond `docs` — a changelog, blog, or API reference in its
  own collection gets the same Markdown routes and index coverage
...
to go on falls back to windows-1252, and every smart quote and em dash in the
page renders as mojibake. The file on disk is fine. The fix is one header
parameter.

Whatever your host is, make sure it serves `.md` as `text/markdown;
charset=utf-8`. Most hosts take that as a headers config. `aws s3 sync` is the
awkward exception, because the type is baked in at upload time and a second
single-pass run will not correct it. That one needs two passes. Note also that
`astro preview` serves `.md` with no charset, so a local check looks identical
to the broken production case.

The [Deployment
guide](https://holdenhewett.github.io/starlight-llm-actions/guides/deployment/)
has the worked example for both, including why `--delete` stays safe across the
two passes.
